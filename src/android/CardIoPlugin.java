    ...
    private boolean scanCardLogic(JSONArray args) {
        if (CardIOActivity.canReadCardWithCamera()) {
            try {
                ...
                boolean suppressConfirmation = args.length() >= 8 ? args.getBoolean(7) : false;
                boolean scanExpiry = args.length() >= 9 ? args.getBoolean(8) : false;
                //String guideColor = args.length() >= 10 ? args.getString(9) : "#00FF00";
                ...
                scanIntent.putExtra(CardIOActivity.EXTRA_SUPPRESS_CONFIRMATION, suppressConfirmation); // default: false
                scanIntent.putExtra(CardIOActivity.EXTRA_SCAN_EXPIRY, scanExpiry); // default: false
